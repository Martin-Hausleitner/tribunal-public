...
|---|---|---|
| Canonical NotebookLM URL strictness | Critique: validator accepts queries/fragments and should block | **Accepted and fixed.** The CLI reproduced an exit-0 structural score for `?unexpected=1#fragment`. `validate_notebooklm_url` now rejects params/query/fragment, the test covers both variants, and the malformed case exits 2 without traceback. |
