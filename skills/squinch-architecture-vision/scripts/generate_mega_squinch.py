...
# Compact semantic blueprint. The generated .squinch is the canonical render input in CI;
# private/source evidence stays in the host repositories, not in this public blueprint.
SYSTEMS = [
("entry","Agent-Einstieg & Heartbeat","blue","sys/log-in",[
...
("hans","Hans — Git Second Brain","green","sys/brain",[
...
("agent","Agent-Concept — Routing & Orchestration","violet","sys/bot",[
...
("tribunal","Tribunal — Independent Review","pink","sys/scale",[
...
("browser","Browser & UI Projection","blue","sys/monitor-up",[
...
("matrix","Matrix Communication","violet","sys/messages-square",[
...
("observability","Observability & Status","teal","sys/chart-no-axes-combined",[
...
("security","Security, Privacy & Authority","red","sys/shield",[
...
("research","Research & Knowledge Intelligence","teal","sys/search-check",[
...
def emit(out:Path):
    ...
    L += ['','view landscape {','  title "⚡ Hans + Agent-Concept + Omni Audio Matrix + Tribunal — Mega Architecture"','  include *','  legend auto','  titleblock {','    subtitle: "Git-first · Skills · Audio · Tribunal · Evidence · Enterprise"','    version: "2026-09-05"','    owner: "Martin"','  }','  note top-right "Click into systems. HTML is canonical; screenshots are visual-QA evidence." { style: info }','  layout {','    direction down','    density compact','    lines orthogonal','    rows [entry hans agent oam tribunal openspec]','    rows [skills browser matrix gitfabric evidence execution]','    rows [observability security integrations lifecycle company research]','  }','}','']
    for sid,label,_,_,_ in SYSTEMS:
        L += [f'view {sid} {{',f'  title "{label} — Detail"',f'  scope {sid}','  expand *','  show descriptions','  legend auto','  layout {','    direction down','    density compact','    lines orthogonal','  }','}','']
    curated={
      'endtoend':('End-to-End — Issue → Integration → Wissen',['entry','hans','agent','tribunal','openspec','execution','evidence','gitfabric','browser','matrix','lifecycle']),
      'controlplane':('Control Plane — Claims, Authority, Review, Status',['entry','agent','tribunal','gitfabric','security','observability','lifecycle']),
      'audioflow':('Audio Intelligence — Capture → Context → Review → Hans',['oam','evidence','tribunal','hans','browser','matrix','research']),
      'visualqa':('Visual Architecture QA — Squinch → HTML → Screenshots → Vision → Iterate',['skills','browser','research','evidence','gitfabric','tribunal']),
      'enterprise':('Enterprise Departments & Delivery',['company','hans','agent','openspec','execution','security','observability','tribunal']),
      'knowledgeflow':('Knowledge & Research Flow',['research','hans','evidence','tribunal','lifecycle','company']),
    }
    for name,(title,items) in curated.items():
        L += [f'view {name} {{',f'  title "{title}"','  only '+', '.join(items),'  show descriptions','  legend auto','  layout {','    direction right','    density compact','    lines orthogonal','  }','}','']
    L += ['view everything {','  title "🧬 Everything — Full Expanded Architecture"','  include *','  expand *','  show descriptions','  legend auto','  note top-right "Exhaustive engineering view; use scoped views for local reasoning." { style: warning }','  layout {','    direction down','    density compact','    lines orthogonal','  }','}']
    out.write_text('\n'.join(L)+'\n')
